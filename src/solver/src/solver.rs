...
use {
    ...
    batsat as sat,
    batsmt_core::{ast_u32::AST, backtrack},
    batsmt_theory::{self as theory, Ctx, LitMap, Theory, TheoryClauseRef, TheoryLit, Trail},
    std::{fmt, marker::PhantomData},
    tracing::{span, Level},
};

pub use {crate::blit::BLit, batsat::lbool, batsmt_theory::LitMapBuiltins as Builtins};
...
fn get_or_create_lit_<C, F>(ctx: &C, lit_map: &mut SatLitMap, l: TheoryLit<C>, f: F) -> BLit
where
    C: Ctx<B = BLit>,
    F: FnOnce() -> BLit,
{
...
mod solver {
    use {super::*, batsat::SolverInterface, batsmt_pretty::Pretty1, batsmt_theory::LitMap};
        ...
        Th: Theory<C>,
    {
        ...
        /// Access statistics.
        pub fn th_stats(&self) -> &theory::Stats {
            &self.s0.c.th_stats
        }

        /// Access literal map of this solver.
        #[inline(always)]
        pub fn lit_map(&self) -> &SatLitMap {
            &self.s0.c.lit_map
        }

        /// Access literal map of this solver.
        #[inline(always)]
        pub fn lit_map_mut(&mut self) -> &mut SatLitMap {
            &mut self.s0.c.lit_map
        }
        ...
        pub fn new_term_lit(&mut self, ctx: &mut C, t: AST) -> BLit {
            ...
            let f = || BLit::from_var(sat.new_var_default(), true);
        ...
        pub fn add_clause(&mut self, m: &C, c: TheoryClauseRef<C>) {
            ...
            self.lits.clear();
            let s0 = &mut self.s0;
            self.lits.extend(c.iter().map(|lit| {
        ...
        pub fn solve_with(&mut self, m: &mut C, assumptions: &[sat::Lit]) -> Res {
            let _sp = span!(Level::INFO, "solver.sat.solve");
        ...
        */
        ...
        pub fn n_lits(&self) -> usize {
            self.s0.sat.num_vars() as usize
        }
        pub fn n_clauses(&self) -> usize {
            self.s0.sat.num_clauses() as usize
        }
        pub fn n_conflicts(&self) -> usize {
            self.s0.sat.num_conflicts() as usize
        }
        pub fn n_props(&self) -> usize {
            self.s0.sat.num_propagations() as usize
        }
        pub fn n_decisions(&self) -> usize {
            self.s0.sat.num_decisions() as usize
        }
    }
        ...
        Th: Theory<C>,
    {
        ...
        fn explain_propagation(&mut self, p: sat::Lit) -> &[sat::Lit] {
            ...
            let (t, sign) = {
                ...
                debug_assert!(
                    t_opt.is_some(),
                    "try to explain {:?} which is not a theory lit",
                    p
                );
                t_opt.unwrap()
            };
            let e = self
                .0
                .th
                .explain_propagation(self.1, t, sign, blit)
                .iter()
                .map(|l| l.0);
    ...
    impl Cb {
        ...
        fn stats<'a>(&'a self) -> impl fmt::Display + 'a {
            self
        }
    }
