...
use {
    ...
    crate::{
        ...
        theories::SVec,
        Ctx, pp_t, ConstructorView, HasConstructor,
    },
};
    ...
    where C: Ctx + HasConstructor<AST>
{
    ...
    fn on_new_term(&mut self, c: &mut C, cc1: &mut cc::CC1<C>, t: &AST, n: NodeID) {
        match c.view_as_constructor(t) {
            ConstructorView::AppConstructor(f, _) => {
                // add to the table
            ...
            ConstructorView::Select {f, idx:_, sub} => {
    ...
    fn before_merge(&mut self, c: &mut C, acts: &mut MicroTheoryArg<C>, a: NodeID, b: NodeID) {
        ...
        for (sel_t1, n1, inj_t2, n2) in pending_sel {
            ...
            let (f, idx, sub) = match c.view_as_constructor(&sel_t1) {
                ConstructorView::Select{f, idx, sub} => { (f.clone(), idx, sub) },
                _ => unreachable!(),
            };
            ...
            let inj_t2_idx = match c.view_as_constructor(&inj_t2) {
                ConstructorView::AppConstructor(f2, args) => {
                    ...
                    (Some((f1,t1)), Some ((f2,t2))) => {
                        if f1 == f2 {
                            ...
                            match (c.view_as_constructor(&t1), c.view_as_constructor(&t2)) {
                                (ConstructorView::AppConstructor(f_t1, args1),
                                ConstructorView::AppConstructor(f_t2, args2)) => {
